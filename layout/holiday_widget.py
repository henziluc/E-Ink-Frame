...
def display_holiday(draw, df, x_start, y_start):
    ...
    for _, row in df[df['start_date'] > now].head(3).iterrows():
        ...
        if next_holiday == 1:
            draw_centered_text(draw, row['location'], (x_start , y, x_start + 250, y + 30), font_medium, fill_main)
            y += spacing_medium
            draw_centered_text(draw, f"{days} days to go", (x_start, y, x_start + 250, y + 25), font_normal, fill_main)
            next_holiday = 0
            y += spacing_normal
